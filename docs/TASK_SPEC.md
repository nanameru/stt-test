...
**デスクトップアプリ型**での実装に向けた、**最適な音声認識APIおよびモデルを選定**するため。

## 要件

- **リアルタイム** or **1〜2秒遅延**でできるAPI/処理機構
- **Electronデスクトップアプリ**として動作（Next.js + Electron）
- マイクAPI（getUserMedia）を使用したリアルタイム音声入力
...
### コア機能
1. **リアルタイム音声入力** - getUserMediaでマイクから音声をキャプチャ
2. **STT API統合** - 各APIへの音声ストリーミング送信
3. **文字起こし結果のリアルタイム表示** - 各APIの結果を並列表示
4. **遅延時間の測定** - 発話から文字起こし表示までの時間を計測
5. **Electronデスクトップアプリ** - クロスプラットフォーム対応

### オプション機能
6. **話者分離（Diarization）表示** - Speaker A, B等のラベル付き表示
7. **話者割り当て** - Geminiを使用した話者推定
8. **評価結果の記録・比較** - 各APIの評価表を自動生成
